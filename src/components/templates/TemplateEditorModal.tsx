        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 overflow-y-auto max-h-[75vh]">
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[11px] text-slate-400">Tokens supported</span>
            </div>
            <input
              ref={subjectInputRef}
              id="template-subject"
          ...
          <div className="bg-slate-50/80 border border-slate-200/70 rounded-xl p-3 space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-xs font-medium text-slate-600 flex items-center gap-1.5">
                <Tag className="w-3.5 h-3.5 text-blue-600" />
                Insert dynamic variables ({activeField === 'subject' ? 'into subject' : 'into body'}):
              </span>
            </div>
            <div className="flex flex-wrap gap-1.5">
              {TEMPLATE_VARIABLES.map((v) => (
                <button
                  ...
                  title={v.description}
                  className="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-mono font-medium rounded-lg bg-white border border-slate-200 hover:border-blue-300 hover:bg-blue-50/50 hover:text-blue-700 text-slate-700 transition-all cursor-pointer shadow-2xs"
                >
                  <span className="text-blue-600 font-bold">+</span>
                  <span>{v.tag}</span>
                  <span className="text-[10px] text-slate-400 font-sans ml-1">({v.label})</span>
                </button>
              ))}
            </div>
          </div>
